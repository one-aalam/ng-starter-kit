     ...
     <div class="py-2 flex flex-col place-items-center">
        ...
        <div v-else>
            <div class="mt-2 text-center">
                <lib-avatar [src]="profile?.avatar_url || '' | safe:'url'" [title]="profile?.username || ''" [loading]="avatarLoading" (change)="updateAvatar($event)"></lib-avatar>
            ...
            <div class="profile-detail my-4 flex flex-col place-items-center">
                    ...
                    <button class="text-gray-500 text-sm my-1" (click)="open()">(update profile)</button>
            </div>
        </div>
    </div>
  `,
...
export class ProfileComponent implements OnInit {
  ...
  constructor(private profileService: ProfileService, private alertService: AlertService, private dialog: DialogService) { }
  ...
  async getProfile() {
    try {
      ...
      if (profile) {
        ...
        this.profile.avatar_url = await this.profileService.getAvatar(this.profile.avatar_url)
  ...
  async updProfile({ username, website }: Omit<Profile, 'avatar_url'>): Promise<void> {
        try {
            ...
            // @ts-ignore
            let { error: updateError } = await this.profileService.updateProfile({ username, website })
            if (updateError) {
                throw updateError
            }
